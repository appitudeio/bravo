...
function copyBootstrap() {
  ...
  // Check if Bravo's dist folder exists, if not use Bootstrap's dist as fallback
  if (fs.existsSync(source)) {
    fs.mkdirSync(destination, { recursive: true })
    fs.cpSync(source, destination, { recursive: true })
  } else {
    // Fallback to Bootstrap's dist files from node_modules
    const bootstrapDist = path.join(process.cwd(), 'node_modules', 'bootstrap', 'dist')
    if (fs.existsSync(bootstrapDist)) {
      fs.mkdirSync(destination, { recursive: true })
      fs.cpSync(bootstrapDist, destination, { recursive: true })
      console.log('Warning: Using Bootstrap dist as fallback. Consider building Bravo dist files.')
    } else {
      console.error('Error: Neither Bravo dist nor Bootstrap dist found!')
    }
  }
}
